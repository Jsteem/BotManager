...
public class RuneLite extends net.runelite.client.RuneLite {
    ...
    public static ArgumentAcceptingOptionSpec<?>[] handleParsing(OptionParser parser) {
        ...
        final ArgumentAcceptingOptionSpec<String> accountInfo = parser
                .accepts("account", "Account info")
                .withRequiredArg().ofType(String.class);


        return (ArgumentAcceptingOptionSpec<?>[]) new ArgumentAcceptingOptionSpec[]{sessionfile, configfile, updateMode, proxyInfo, accountInfo};
    }
    ...
    public static void initializeClient(ArgumentAcceptingOptionSpec<?>[] optionSpecs, OptionSet options) {
        ...
        try
        {
            ...
            if (options.has("account")) {
                String[] account = options.valueOf(optionSpecs[Options.ACCOUNT_INFO.getIndex()].ofType(String.class)).split(":");
                ...
                if (account.length >= 3) {
                     startScript = account[2];
                    System.out.println("Start Script: " + startScript);
                }

                if (account.length >= 4) {
                    isMember = Boolean.parseBoolean(account[3]);
            ...
            injector = Guice.createInjector(new BotModule(
                    ...
                    login, password, startScript, isMember));
     ...
     */
    enum Options {
        SESSION_FILE(0),CONFIG_FILE(1), UPDATE_MODE(2), PROXY_INFO(3), ACCOUNT_INFO(4);
