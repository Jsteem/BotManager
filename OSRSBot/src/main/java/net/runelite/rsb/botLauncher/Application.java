...
import net.runelite.rsb.common.AccountDTO;
import net.runelite.rsb.common.GetRequest;
...
public class Application {
	...
	public static void addBot(boolean headless) {
		...
		try {
			GetRequest<AccountDTO> accountGetRequest = new GetRequest<>("accounts/account", AccountDTO.class);
			AccountDTO accountDTO = accountGetRequest.getResult();
			if(accountDTO != null && !preParser.containsNoDelete("--proxy") && !preParser.containsNoDelete("--account")){

				if(accountDTO.getProxyIp() != null
						&& accountDTO.getProxyPassword() != null
						&& accountDTO.getProxyPort() != null
						&& accountDTO.getProxyUser() != null){

					preParser.add("--proxy");


					String proxyValue = String.format("%s:%s:%s:%s",
							accountDTO.getProxyIp(),
							accountDTO.getProxyPort(),
							accountDTO.getProxyUser(),
							accountDTO.getProxyPassword());


					preParser.add(proxyValue);

				}



				preParser.add("--account");



				String accountValue = String.format("%s:%s:%s:%s",
						accountDTO.getLogin(),
						accountDTO.getPassword(),
						accountDTO.getStartScript(),
						accountDTO.getIsMember());
				preParser.add(accountValue);
			}
	...
	private static class ArgumentPreParser extends ArrayList<String> {
		...
		public boolean containsNoDelete(Object o){
			int index = indexOf(o);
			boolean within = index >= 0;
			return within;
		}
